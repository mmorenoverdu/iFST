...
This experiment is built using [PsychoPy](https://www.psychopy.org/) (Builder) and is intended for **local, in-lab execution**. Please make sure you are running PsychoPy 2025.2.4 or later, as other versions might behave unexpectedly.
...
**Requirements:** PsychoPy 2025.2.4 or later. No additional dependencies for local use.
...
5. The output `.csv` file is saved automatically in the `data/` subfolder.
6. **Process the data** using the `Example data processing.R` script provided.
...
> **⚠️ Cloud sync:** If the experiment folder is inside a cloud-synced location (e.g., OneDrive), **pause synchronisation before running** to avoid data corruption.

---------------------------------------

## LANGUAGE LOCALISATION

This experiment uses two external spreadsheet files to manage on-screen text and translations: `language_localiser.xlsx` and `messages.xlsx`. The language is selected via the PsychoPy startup dialog (`language` field) before every run. The name entered must exactly match an entry in the `language` column of `language_localiser.xlsx`.
...
In PsychoPy: **Experiment Settings** (cogwheel) → Basic → Experiment Info → update the `language` dropdown to include the new language name. It must exactly match the entry in `language_localiser.xlsx`.
...
│   └── incorrect_sound.wav
│
├── data/                           ← Output data (created automatically on first run)
...
- The experiment is built entirely in **PsychoPy Builder**; all custom logic is in code components with Python (local) and JavaScript (online) tabs.
- `iFST_lastrun.py` is auto-generated by PsychoPy on each run. Do not edit it directly.
...
Go to **Experiment Settings** (cogwheel) → Screen → uncheck **Full-screen window**. Re-enable before data collection.
...
1. **Language selection:** Selected via the startup dialog before the experiment launches.
...
Output is saved as a `.csv` file in the `data/` subfolder, named `<participant>_<date>.csv`. It contains every variable encoded in the experiment — one row per trial event (trials and self-assessment responses are interleaved).
...
| `seq_resp.keys` | list | Keys pressed by the participant (Python list format). |
| `seq_resp.rt` | list | Response times in seconds for each keypress (Python list format). Trial duration = last − first value. |
...
Before collecting data, test the display geometry, key responses, localised text, and CSV output on the target hardware. PsychoPy version updates may require adjustments to the experiment file.
